...
def register():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("SELECT count(id) as count from users where username = :name", name=request.form.get("username"))
...
def sell():
    ...
    if request.method == "POST":
        ...
        # Book keeping (TODO: should be wrapped with a transaction)
        db.execute("UPDATE users SET cash = cash + :price WHERE id = :user_id", price=total_price, user_id=session["user_id"])
        db.execute("UPDATE transactions SET shares = shares - :shares WHERE user_id = :user_id",
                   shares=shares, user_id=session["user_id"])
